...
A partir da v2.18.2, o fluxo recomendado ficou mais simples:

1. Publique a nova versão no Cloudflare Pages.
2. Entre no eMulti como Administrador.
3. Abra **Administração > Diagnóstico**.
4. Aplique o `database/update.sql` no banco `regulacao-vagas-db` quando esta versão exigir atualização estrutural.
